...
export function ZakatDrawer({ open, onOpenChange }: ZakatDrawerProps) {
    ...
    return (
        <Drawer open={open} onOpenChange={onOpenChange}>
            <DrawerContent className="bg-[#EBE7DF] border-none max-w-md mx-auto shadow-2xl">
                ...
                <div className="px-6 py-4 space-y-5">
                    <div className="space-y-2">
                        ...
                        <Input
                            ...
                            className="bg-[#FAFAFA] border-[#DCD6CA] focus:ring-[#495C48] text-[#3E5345] font-medium h-12 rounded-xl text-[15px] px-4 shadow-[0_2px_10px_rgba(0,0,0,0.02)]"
                        />
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <Select onValueChange={(v) => setRate(parseInt(v))} defaultValue="8">
                            <SelectTrigger className="bg-[#FAFAFA] border-[#DCD6CA] text-[#3E5345] font-medium h-12 rounded-xl text-[14px] px-4 shadow-[0_2px_10px_rgba(0,0,0,0.02)]">
                                <SelectValue placeholder="Select rate" />
                            </SelectTrigger>
                            <SelectContent className="bg-[#FAFAFA] border-[#DCD6CA] text-[#3E5345] font-medium rounded-xl">
                                {RATES.map((r) => (
                                    <SelectItem key={r.value} value={r.value.toString()} className="focus:bg-[#EBE7DF]">
                                        {r.label} - RM {r.value}
                                    </SelectItem>
                                ))}
                            </SelectContent>
                    ...
                    <div className="bg-[#FAFAFA] border border-white rounded-[1.75rem] p-6 text-center shadow-[0_4px_20px_rgba(0,0,0,0.03)] mt-2">
