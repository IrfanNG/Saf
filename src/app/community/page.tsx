...
export default function CommunityPage() {
    ...
    // Lost & Found data
    const { items, loading: lostLoading, addItem, markReturned, deleteItem } = useLostAndFound();
    ...
    return (
        <main className="min-h-screen pb-32 pt-12 px-5 bg-[#F4F4F6]">
            <div className="flex flex-col gap-1 max-w-4xl mx-auto">
                ...
                <div className="flex bg-[#4D6A53] rounded-[1.25rem] p-1.5 mb-7 shadow-[inset_0_2px_5px_rgba(0,0,0,0.08)]">
                    <button
                        ...
                        className={`flex-1 text-[13px] font-bold py-[0.65rem] rounded-[1rem] transition-all duration-300 ${activeTab === 'feed' ? 'bg-white text-[#4D6A53] shadow-[0_2px_8px_rgba(0,0,0,0.08)]' : 'text-white/60 hover:text-white'}`}
                    >
                        Community Feed
                    </button>
                    <button
                        onClick={() => setActiveTab("lost-found")}
                        className={`flex-1 text-[13px] font-bold py-[0.65rem] rounded-[1rem] transition-all duration-300 ${activeTab === 'lost-found' ? 'bg-white text-[#4D6A53] shadow-[0_2px_8px_rgba(0,0,0,0.08)]' : 'text-white/60 hover:text-white'}`}
                    >
                        Lost & Found
                    </button>
                </div>
                        ...
                        >
                            ...
                            {feedLoading ? (
                                ...
                                <div className="flex flex-col items-center justify-center py-32 text-center bg-white/40 border border-dashed border-slate-200 rounded-[2.5rem]">
                                    ...
                                    <h2 className="text-lg font-bold text-[#1A1A1A]">No posts yet</h2>
                                    <p className="text-slate-400 font-medium text-sm mt-1 max-w-[200px]">Be the first to share something with the community.</p>
                                </div>
                            ) : (
            ...
            <div className="fixed bottom-28 right-6 z-50">
                <Button
                    onClick={() => activeTab === 'feed' ? setIsPostDialogOpen(true) : setIsLostDialogOpen(true)}
